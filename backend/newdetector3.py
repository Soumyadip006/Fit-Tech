...
# Five semantic roots plus numbered versions of the first three roots
# produce 32+ accepted names per exercise. Matching remains case- and
# punctuation-insensitive through _normalise_filename().
ALIAS_ROOTS = { 
    "squat": ["squat", "squats", "squat_video", "legsquat", "back_squat"],
    "deadlift": ["deadlift", "dead_lift", "deadlifts", "hip_hinge", "barbell_deadlift"],
    "lunge": ["lunge", "lunges", "forward_lunge", "leg_lunge", "split_lunge"],
    "push up": ["pushup", "push_up", "push-up", "pushups", "pressup"],
    "plank": ["plank", "planks", "front_plank", "core_plank", "forearm_plank"],
    "pull up": ["pullup", "pull_up", "pull-up", "pullups", "chinup"],
    "sit up": ["situp", "sit_up", "sit-up", "situps", "ab_situp"],
    "leg raise": ["legraise", "leg_raise", "leg-raise", "legraises", "hanging_leg_raise"],
    "dead hang": ["deadhang", "dead_hang", "dead-hang", "dead_hangs", "hang"],
    "bent over row": ["bentoverrow", "bent_over_row", "bentover_row", "bent_row", "bent"],
    "overhead press": ["overheadpress", "overhead_press", "ohp", "shoulderpress", "shoulder_press"],
}


def build_aliases(roots):
    aliases = list(roots)
    for root in roots:
        aliases.extend(f"{root}{number}" for number in range(1, 11))
    return aliases


VIDEO_ALIASES = {exercise: build_aliases(roots)
                 for exercise, roots in ALIAS_ROOTS.items()}
...
VIEW_PROFILES = {
    "side": {
        "description": "Sagittal-plane analysis: depth, hip hinge, torso lean, and elbow travel.",
        "primary_checks": "knee/hip angles, torso lean, and movement range",
    },
    "front": {
        "description": "Frontal-plane analysis: left-right symmetry, knee tracking, and shoulder level.",
        "primary_checks": "left-right symmetry, knee alignment, and shoulder alignment",
    },
    "back": {
        "description": "Posterior-plane analysis: back alignment, shoulder symmetry, and pelvic symmetry.",
        "primary_checks": "back alignment, shoulder symmetry, and pelvic alignment",
    },
}
...
def choose_view():
    while True:
        value = input("  Select camera view (side/front/back): ").strip().lower()
        aliases = {"s": "side", "f": "front", "b": "back"}
        value = aliases.get(value, value)
        if value in VIEW_PROFILES:
            return value
        print("  Invalid view. Please enter side, front, or back.")
...
def analyze_video(VIDEO_PATH, requested_exercise=None, selected_view="side"):
    selected_view = selected_view.lower().strip()
    if selected_view not in VIEW_PROFILES:
        raise ValueError("selected_view must be side, front, or back")
    ...
    print(f"  CAMERA VIEW        : {selected_view.upper()}")
    print(f"  VIEW ANALYSIS      : {VIEW_PROFILES[selected_view]["primary_checks"]}")
    if requested_exercise:
    ...
    # ============================================================
    print(f"\n  VIEW-SPECIFIC CHECKS ({selected_view.upper()})")
    if selected_view == "side":
        print(f"  Sagittal measurements: hip range {hip_range:.1f}, knee range {knee_range:.1f}, torso lean {avg_torso_lean:.1f} degrees")
        print("  Best for judging depth, hinge, torso angle, and front-to-back movement.")
    elif selected_view == "front":
        print(f"  Left/right knee asymmetry: {knee_asymmetry:.1f} degrees")
        print(f"  Left/right knee-height asymmetry: {knee_y_asymmetry:.3f}")
        print(f"  Left/right shoulder angle difference: {abs(np.mean(data['left_shoulder']) - np.mean(data['right_shoulder'])):.1f} degrees")
        print("  Best for judging symmetry and side-to-side alignment; depth estimates are secondary.")
    else:
        print(f"  Back-view knee asymmetry: {knee_asymmetry:.1f} degrees")
        print(f"  Back-view hip-height asymmetry: {abs(np.mean(data['left_hip']) - np.mean(data['right_hip'])):.3f}")
        print(f"  Back-view shoulder-angle difference: {abs(np.mean(data['left_shoulder']) - np.mean(data['right_shoulder'])):.1f} degrees")
        print("  Best for judging back, shoulder, and pelvic symmetry; sagittal depth is secondary.")
...
print()
print("  View tip: choose side, front, or back after selecting an exercise")
...
while True:
    ...
    if user_input in VIDEO_ALIASES:
        ...
        if video_path:
            ...
            analyze_video(video_path, requested_exercise=user_input, selected_view=selected_view)
        else:
